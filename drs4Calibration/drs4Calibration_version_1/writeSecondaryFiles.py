...
@click.argument('store_folder_path',
                default=('/net/big-tank/POOL/projects/fact/' +
                         'drs4_calibration_data/calibration/validating/version_1/meanAndStd/interval3/'),
                type=click.Path(exists=True))
...
@click.argument('time_interval',
                default=['2017-02-02', '2017-10-01'])  # ['2014-05-20', '2015-05-26'], ['2015-05-26', '2017-10-01']
###############################################################################
def drs_pedestal_run_mean_and_std(source_folder_path, store_folder_path,
    ...
    store_folder_path_v0 = ('/net/big-tank/POOL/projects/fact/' +
                            'drs4_calibration_data/calibration/validating/version_0/meanAndStd/interval3/')

    existing_file_date_str_list_v0 = sorted([file.strip('pedestelStats_').strip('.fits') for file in
                                            os.listdir(store_folder_path_v0)
                                            if(file.startswith('pedestelStats') and
                                               file.endswith('.fits'))])

    existing_file_date_str_list = sorted([file.strip('pedestelStats_').strip('.fits') for file in
                                         os.listdir(store_folder_path)
                                         if(file.startswith('pedestelStats') and
                                            file.endswith('.fits'))])
    ...
    for date in tqdm(pd.date_range(start=time_interval[0], end=time_interval[1], freq='1D')):

        date_str = date.strftime('%Y%m%d')

        print(date_str)
        # if(date_str not in existing_file_date_str_list_v0):
        #     # print('continue ', date_str)
        #     continue
        # if(date_str in existing_file_date_str_list):
        #     # print('continue ', date_str)
        #     continue
        ...
        drs_calibrated_data_JSremoved_mean = []
        drs_calibrated_data_JSremoved_std = []
        drs_calibrated_data_mean_temp = []
        drs_calibrated_data_std_temp = []
        drs_calibrated_data_JSremoved_mean_temp = []
        drs_calibrated_data_JSremoved_std_temp = []
        ...
        for run_id in existing_pedestal_run_ids:
            ...
            try:
                with fits.open(source_file) as stats_tab:
                    ...
                    data_JSremoved_mean = stats_tab[1].data['DRSCalibratedData_JSremoved_Mean'].flatten()
                    data_JSremoved_std = stats_tab[1].data['DRSCalibratedData_JSremoved_Std'].flatten()
                    ...
                    data_JSremoved_mean_temp = stats_tab[1].data['DRSCalibratedData_Temp_JSremoved_Mean'].flatten()
                    data_JSremoved_std_temp = stats_tab[1].data['DRSCalibratedData_Temp_JSremoved_Std'].flatten()

                if((len(data_mean) == data_shape) and (len(data_std) == data_shape) and
                   (len(data_JSremoved_mean) == data_shape) and (len(data_JSremoved_std) == data_shape) and
                   (len(data_mean_temp) == data_shape) and (len(data_std_temp) == data_shape) and
                   (len(data_JSremoved_mean_temp) == data_shape) and (len(data_JSremoved_std_temp) == data_shape)):
                        drs_calibrated_data_mean.append(data_mean)
                        drs_calibrated_data_std.append(data_std)
                        drs_calibrated_data_JSremoved_mean.append(data_JSremoved_mean)
                        drs_calibrated_data_JSremoved_std.append(data_JSremoved_std)
                        drs_calibrated_data_mean_temp.append(data_mean_temp)
                        drs_calibrated_data_std_temp.append(data_std_temp)
                        drs_calibrated_data_JSremoved_mean_temp.append(data_JSremoved_mean_temp)
                        drs_calibrated_data_JSremoved_std_temp.append(data_JSremoved_std_temp)
        ...
        tbhduStats = fits.BinTableHDU.from_columns(
                [fits.Column(
                 ...
                 fits.Column(
                    name='DRSCalibratedData_JSremoved_Mean', format='{}E'.format(data_shape),
                    unit='mV', array=drs_calibrated_data_JSremoved_mean),
                 fits.Column(
                    name='DrsCalibratedData_JSremoved_Std', format='{}E'.format(data_shape),
                    unit='mV', array=drs_calibrated_data_JSremoved_std),
                 fits.Column(
                 ...
                 fits.Column(
                    name='DrsCalibratedData_JSremoved_Temp_Mean', format='{}E'.format(data_shape),
                    unit='mV', array=drs_calibrated_data_JSremoved_mean_temp),
                 fits.Column(
                    name='DrsCalibratedData_JSremoved_Temp_Std', format='{}E'.format(data_shape),
                    unit='mV', array=drs_calibrated_data_JSremoved_std_temp)])
...
@click.argument('store_folder_path',
                default=('/net/big-tank/POOL/projects/fact/' +
                         'drs4_calibration_data/calibration/validating/version_1/meanAndStd/outside/'),
                type=click.Path(exists=True))
@click.argument('facttools_file_path',
                default=('/home/fschulz/git/fact-tools_drs/' +
                         'target/fact-tools-0.18.1-SNAPSHOT.jar'),
...
@click.argument('facttools_xml_path',
                default=('/home/fschulz/git/fact-tools_drs/' +
                         'examples/drsCalibration/drsCalibrationMeanAndStd_outside.xml'),
                type=click.Path(exists=True))
@click.argument('fitparameter_file_path_temp',
                default=('/net/big-tank/POOL/projects/fact/' +
                         'drs4_calibration_data/calibration/calculation/' +
                ...
                type=click.Path(exists=True))
@click.argument('time_interval',
                default=['2018-01-01', '2018-01-31'])  # ['2014-05-20', '2015-05-26'], ['2015-05-26', '2017-10-01']
###############################################################################
def drs_pedestal_run_mean_and_std_outside(source_folder_path, store_folder_path,
                                          facttools_file_path, facttools_xml_path,
                                          fitparameter_file_path_temp, time_interval):
    jobs = 3
    verbosity = 0
    ...
    for date in tqdm(pd.date_range(start=time_interval[0], end=time_interval[1], freq='1D')):
        ...
        pool(delayed(run_fact_tools_outside)(
             facttools_file_path,
             facttools_xml_path,
             existing_pedestal_run_files[run_index],
             store_file_list[run_index],
        ...
        for run_id in existing_pedestal_run_ids:
            ...
            try:
                ...
                if((len(data_mean_temp) == data_shape) and (len(data_std_temp) == data_shape) and
                   (len(data_JSremoved_mean_temp) == data_shape) and (len(data_JSremoved_std_temp) == data_shape)):
                        drs_calibrated_data_mean_temp.append(data_mean_temp)
                        drs_calibrated_data_std_temp.append(data_std_temp)
                        drs_calibrated_data_JSremoved_mean_temp.append(data_JSremoved_mean_temp)
        ...
        tbhduStats = fits.BinTableHDU.from_columns(
                [fits.Column(
                 ...
                 fits.Column(
                    name='DrsCalibratedData_JSremoved_Temp_Std', format='{}E'.format(data_shape),
                    unit='mV', array=drs_calibrated_data_JSremoved_std_temp)])
        tbhduStats.header.insert('TFIELDS', ('EXTNAME', 'StatsPerPixel'), after=True)
        commentStr = ('-')
        tbhduStats.header.insert('EXTNAME', ('comment', commentStr), after='True')
        tbhduStats.header.insert('comment', ('Date', date_str, 'Date yyyy-mm-dd'), after=True)

        store_file_path = store_folder_path+pre_filename+date_str+'.fits'
        print('Save Table')
        thdulist = fits.HDUList([fits.PrimaryHDU(), tbhduStats])
        thdulist.writeto(store_file_path, overwrite=True, checksum=True)
        print('Verify Checksum')
        ...
        try:
            hdul = fits.open(store_file_path, checksum=True)
            print(hdul['StatsPerPixel'].header)
        except Exception as errInfos:
            errorStr = str(errInfos)
            print(errorStr)
...
###############################################################################
def run_fact_tools_outside(facttools_file_path, facttools_xml_path, run_file,
                           store_file_path, pre_aux_path,
                           fitparameter_file_path_temp):
