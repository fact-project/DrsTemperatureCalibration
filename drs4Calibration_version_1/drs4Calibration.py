...
from constants import NRCHID, NRCELL, ROI, NRTEMPSENSOR, DACfactor
from config import data_collection_config, fit_value_config
...
def store_drs_attributes(list_of_needed_files_doc_path: str,
    ...
    for file_collection_of_the_day in calibration_file_list:
...
def get_mean_and_std_for_ROI_1024(fits_stream):
    ...
    for event in fits_stream:
        ...
        np.multiply(delta, delta_new, out=diff, dtype=dtype_data)
        np.add(var_values, diff, out=var_values, dtype=dtype_data)
