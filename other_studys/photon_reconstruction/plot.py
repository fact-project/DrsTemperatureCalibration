...
level = [75, 80, 85, 90, 93, 95, 97, 98, 99, 99.4, 99.7]  # in %
...
fig, axarr = plt.subplots(3, sharex=True, dpi=200, figsize=(6, 8))
...
################################
load_file = '1_photon_reconstruction_rate_'
...
CS_1 = axarr[0].contour(X, Y, Ti, levels=level, colors='black')

# Recast levels to new class
CS_1.levels = [nf(val) for val in CS_1.levels]

axarr[0].clabel(CS_1, CS_1.levels, inline=True, fmt=fmt, fontsize=10)
axarr[0].set_ylabel('Standartabweichung / spe')
axarr[0].set_yticks(np.arange(0.15, 0.35+0.1, 0.1))
axarr[0].set_ylim(0.1, 0.4)
axarr[0].tick_params(direction='inout', top=False, right=False, which='both')

################################
# 2_photon_reconstruction_rate #
################################
load_file = '2_photon_reconstruction_rate_'
with pd.HDFStore(load_file+'.h5') as store:
    df = store['df']
...
axarr[1].set_ylabel('Standartabweichung / spe')
...
################################
load_file = '3_photon_reconstruction_rate_'
with pd.HDFStore(load_file+'.h5') as store:
    df = store['df']
...
CS_3 = axarr[2].contour(X, Y, Ti, levels=level, colors='black')

# Recast levels to new class
CS_3.levels = [nf(val) for val in CS_3.levels]

axarr[2].clabel(CS_3, CS_3.levels, inline=True, fmt=fmt, fontsize=10)
axarr[2].set_ylabel('Standartabweichung / spe')
axarr[2].set_xlabel('Offset / spe')
axarr[2].set_xticks(np.arange(-0.2, 0.2+0.1, 0.05))
axarr[2].set_yticks(np.arange(0.35, 0.55, 0.1))
axarr[2].set_xlim(-0.2, 0.2)
axarr[2].set_ylim(0.3, 0.6)
axarr[2].tick_params(direction='inout', right=False, which='both')
plt.tight_layout()
plt.savefig('photon_reconstruction_rate.png')
plt.show()
